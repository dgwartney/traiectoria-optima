...
| 2 | Stage 1 — Foundation | Compute great-circle edge weights between connected airports using the haversine formula. | ASF | Not Started |
| 3 | Stage 1 — Foundation | Build the `RoutePlanner` graph as an adjacency list (`dict[airport] -> list[(neighbor, weight)]`) loaded from the cleaned data. | ASF | Not Started |
| 4 | Stage 1 — Foundation | Report basic graph statistics (airport count, route count, average degree, disconnected airports, min/max edge distance). | ASF | Not Started |
...
| 7 | Stage 2 — Core algorithm | Implement BFS for fewest-stop (unweighted) routing. | ASF | Not Started |
| 8 | Stage 2 — Core algorithm | Implement a from-scratch binary min-heap (no `heapq`). | TBD | Not Started |
| 9 | Stage 2 — Core algorithm | Implement Dijkstra's algorithm using the custom heap for cheapest/shortest-distance routing. | ASF | Not Started |
...
| 12 | Stage 3 — Stretch + features | Implement the haversine admissible heuristic for A*. | ASF | Not Started |
| 13 | Stage 3 — Stretch + features | Implement A* search using the custom heap and heuristic. | ASF | Not Started |
...
| 19 | Stage 4 — Evaluation | Write the Big-O complexity analysis for the graph build, BFS, Dijkstra, and A*. | ASF | Not Started |
| 20 | Stage 4 — Evaluation | Assemble the final Jupyter notebook (consolidate code, results, and plots into a single runnable notebook). | ASF | Not Started |
...
| 27 | Cross-cutting | Decide on and document airport/route edge cases to handle: multi-airport cities, codeshare duplicate routes, self-loops, and islands/Hawaii/Alaska connectivity. | ASF | Not Started |
...
| 33 | Cross-cutting | Build a small CLI or notebook widget for interactively querying routes (origin, destination, mode) to support the live demo. | ASF | Not Started |
...
2. **[Aastha]** Compute great-circle edge weights between connected airports using the haversine formula.
3. **[Aastha]** Build the `RoutePlanner` graph as an adjacency list (`dict[airport] -> list[(neighbor, weight)]`) loaded from the cleaned data.
4. **[Aastha]** Report basic graph statistics (airport count, route count, average degree, disconnected airports, min/max edge distance).
...
7. **[Aastha]** Implement BFS for fewest-stop (unweighted) routing.
8. Implement a from-scratch binary min-heap (no `heapq`).
9. **[Aastha]** Implement Dijkstra's algorithm using the custom heap for cheapest/shortest-distance routing.
...
12. **[Aastha]** Implement the haversine admissible heuristic for A*.
13. **[Aastha]** Implement A* search using the custom heap and heuristic.
14. [Nice to have] Add secondary query features: fewest-stops vs. cheapest-distance mode switch, and a nearest-airports-to-a-point lookup.
...
19. **[Aastha]** Write the Big-O complexity analysis for the graph build, BFS, Dijkstra, and A*.
20. **[Aastha]** Assemble the final Jupyter notebook (consolidate code, results, and plots into a single runnable notebook).
...
27. **[Aastha]** Decide on and document airport/route edge cases to handle: multi-airport cities, codeshare duplicate routes, self-loops, and islands/Hawaii/Alaska connectivity.
...
- Aastha Sharma-Flores
<<<<<<< Updated upstream
    * As shown above
=======
    * As described above before each task
>>>>>>> Stashed changes
