"""Example: one network, every algorithm, and when a heuristic stops being safe.
...
Swapping the formula changes **no interface** -- that much is the Strategy
pattern working as intended. But it does change something that matters more,
and this demo exists to show it: **admissibility**.

A* only guarantees an optimal path if its heuristic never *overestimates* the
remaining cost. This network's edge weights are hand-typed approximations
rather than computed from the coordinates, so neither formula is guaranteed to
stay under them -- and measured against these weights, Haversine overestimates
on three of five legs and Vincenty on all five. Vincenty is the more physically
accurate formula, and that accuracy is exactly what makes it the worse
heuristic here.

The demo prints the check, then runs both anyway. Both still return Dijkstra's
answer, because six airports is far too small a network for the lost guarantee
to cost anything. **That is the real lesson: an inadmissible heuristic does not
reliably announce itself -- it removes a guarantee, and a small example cannot
tell you the guarantee is gone.**

The project's own graphs do not have this problem: their weights are generated
by the same Haversine formula the heuristic uses, so admissibility holds
exactly. See `flight_planner.geo.heuristic`, and prefer `haversine_heuristic()`
...
from flight_planner.geo import Vincenty, haversine_heuristic
...
if __name__ == "__main__":
    ...
    print("\n--- Is either heuristic admissible on this network? ---")
    # Admissibility is per-edge: the straight-line estimate between an edge's
    # endpoints must not exceed that edge's own weight. These weights were
    # typed by hand, so nothing guarantees it -- unlike the project's real
    # graphs, whose weights are generated by this same Haversine formula.
    haversine = haversine_heuristic()
    vincenty_formula = Vincenty()
    print(f"    {'leg':<12}{'weight':>9}{'haversine':>11}{'vincenty':>10}   over?")
    over_h = over_v = 0
    for leg in planner.edges:
        a, b = leg.origin, leg.destination
        estimate_h = haversine(a, b)
        estimate_v = vincenty_formula.calculate(a, b)
        over_h += estimate_h > leg.distance_km
        over_v += estimate_v > leg.distance_km
        flags = ",".join(
            name
            for name, over in (("haversine", estimate_h > leg.distance_km),
                               ("vincenty", estimate_v > leg.distance_km))
            if over
        )
        print(
            f"    {a.iata_code + '->' + b.iata_code:<12}{leg.distance_km:>9.0f}"
            f"{estimate_h:>11.1f}{estimate_v:>10.1f}   {flags or '-'}"
        )
    print(
        f"    Haversine overestimates on {over_h}/{len(planner.edges)} legs, "
        f"Vincenty on {over_v}/{len(planner.edges)}."
    )
    print("    Neither is admissible here, so neither A* below is guaranteed "
          "optimal.")

    print("\n--- A* (Haversine heuristic): Shortest Path (JFK -> HND) ---")
    ...
    astar_vincenty = AStar(lambda a, b: a.distance_to(b, vincenty_formula))
    ...
    print(
        "\nNOTE: swapping the DistanceFormula changed no interface -- that is the\n"
        "Strategy pattern working. It did change admissibility, which is not a\n"
        "matter of precision. Both runs still matched Dijkstra: six airports is\n"
        "too small for the lost guarantee to cost anything, which is exactly why\n"
        "a small example cannot tell you the guarantee is gone."
    )
